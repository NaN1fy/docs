% changelog: "2.0.0, 2024-08-18, Approvazione per PB"
...
% [colorlinks=true,linkcolor=nan1fyblue,urlcolor=nan1fyblue]
...
\begin{center}\texttt{Visibilità Nome: Tipo<T> [Molteplicità] = Valore/i di Default}\end{center}
    \end{center}
          dove:\\
          \begin{itemize}
            \item \textbf{Visibilità:} precede ogni attributo, e segue questi indicativi:
                \begin{itemize}
                ...
                \end{itemize}
            \item \textbf{Nome:} nome univoco dell'attributo, viene scritto in camelCase per il linguaggio \glossterm{Java}, e in underscore per il linguaggio \glossterm{Python}. Nel caso di costanti, il nome è scritto in Maiuscolo;
            \item \textbf{Tipo:} indica il tipo, primitivo o meno, dell'attributo;
            \item \textbf{$<$T$>$:} indica il tipo di oggetto contenuto nel caso di sequenze o container di elementi, come liste o array;
            \item \textbf{Molteplicità:} indica la lunghezza nel caso di sequenze o container di elementi, come liste o array; se non si conosce a priori la lunghezza degli elementi, si indica con ``[*]". In caso di singolo elemento, questo dato è opzionale;                  
            \item \textbf{Valore/i di Default:} ogni campo dati può essere inizializzato con un valore di default.
          \end{itemize}
    \item \textbf{Firme dei Metodi:} descrivono il comportamento generale dei singoli metodi. Tutti i metodi, anch'essi rappresentati singolarmente in righe separate, seguono il seguente formato:\\
    \begin{center}
 ...
 \begin{itemize}
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.72\textwidth]{images_ndp/aggregation.png}
        \caption{Esempio di aggregazione.}
        \label{fig:Esempio di aggregazione}
    \end{figure}
       \clearpage
    \item \textbf{Composizione:}\\ si dice che una classe è composita con un'altra classe quando la seconda è ``Part of" della prima, ma le classi di cui la prima è composta sono dipendenti da essa e non possono esistere indipendentemente. La composizione viene indicata tramite freccia che inizia con un diamante pieno;
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.72\textwidth]{images_ndp/composition.png}
        \caption{Esempio di composizione.}
        \label{fig:Esempio di composizione}
    \end{figure}
