% changelog: "1.1.0, 2024-07-17, Oscar Konieczny, Stesura settimo periodo"
...
\begin{table}[ht!]	
	...
	\begin{tabular}{p{1.2cm} p{2cm} p{5cm} p{3cm} p{3cm}}
		...
		\midrule
			1.1.0 & 2024-07-17 & Stesura settimo periodo. & Oscar Konieczny & -
 		    \\ % spazio tra le righe
		\bottomrule
	\end{tabular}
...
\subsubsection{Pianificazione}
Il team, in accordo con la \glossterm{Proponente}, ha deciso di svolgere uno \glossterm{Sprint} da tre settimane con un carico di lavoro ridotto, essendo questo un periodo dove i membri del team non riescono a dare pieno impegno alle attività del progetto per via degli impegni accademici dovuti dalla sessione estiva degli esami universitari. \\
L'obiettivo di questo Sprint è di completare attività che sono state prefissate precedentemente e la continuazione di implementazione dell'espansione del prodotto. \\
In particolare, l'implementazione del sistema di alerting nella piattaforma \glossterm{Grafana} richiede ulteriore sviluppo per essere implementato a pieno, ma l'obiettivo principale è quello di implementare le dashboard relative ai nuovi sensori che sono stati creati nello Sprint precedente. Continua anche l'auto-formazione sulla tecnologia Apache Flink.\\
\begin{itemize}
    \setlength{\itemsep}{0em}
    \item Implementazione di nuove dashboard per la visualizzazione dei dati generati dai nuovi sensori;
    \item Termine dell'implementazione del sistema di notifica in Grafana;
...
\begin{table}[ht!]
	...
	\begin{tabular}{p{4cm} p{1cm} p{1cm} p{1cm} p{1cm} p{1cm} p{1cm} p{3cm}}
        ...
        \bottomrule
        \textbf{Totale (ruolo)} & 1     & 4     & 3    & 0    & 10    & 3    & 21 \\
	\end{tabular}
...
\subsubsubsection{Rischi occorsi, impatto e loro mitigazione} \\
Come previsto, i vari membri del team hanno dedicato minor tempo allo svolgimento delle attività del progetto per dedicare più tempo allo studio agli esami universitari. Essendo questo un rischio che è stato considerato nella pianificazione, non pone nessun ritardo o problematica, perciò nessuna mitigazione è necessaria. 
