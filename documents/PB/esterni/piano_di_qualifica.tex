% changelog: "1.3.1, 2024-08-15, Linda Barbiero, Continuazione paragrafi metriche qualità di prodotto."
...
\begin{table}[ht!]	
	...
	\begin{tabular}{p{1.2cm} p{2cm} p{6cm} p{3cm} p{2cm}}
		...
		\midrule
        1.3.1 & 2024-08-15 & Completamento della stesura dei paragrafi metriche di qualità di prodotto. & Linda Barbiero & \\\\
        1.3.0 & 2024-08-15 & Stesura iniziale dei paragrafi metriche di qualità di prodotto. & Linda Barbiero & \\\\
...
\end{figure}
\textbf{\glossterm{PB}:} A differenza dei requisiti precedenti, il team non è riuscito a raggiungere una percentuale di soddisfazione dei requisiti opzionali ottimale. Ciò è stato dovuto ovviamente alla priorità minore di integrazione e dal desiderio della stessa Proponente, la quale ha preferito l'integrazione e la concretizzazione di altri requisiti.
\clearpage
...
\end{figure}
\textbf{\glossterm{PB}:} Durante lo svolgimento degli Sprint il numero medio di click per raggiungere ogni funzionalità, nel nostro caso uno specifico panel, sono sempre rimasti 4: 2 per la sezione di login (compresa una parte di cambio password), 1 di scelta della \glossterm{dashboard} e 1 di selezione del panello preciso
\clearpage
...
\end{figure}
\textbf{\glossterm{PB}:} Così come il numero medio di click, anche il tempo di apprendimento è rimasto generalmente costante. Nonostante il team si aspettasse un tempo di apprendimento inferiore, l'interfaccia grafica integrata di Grafana è risultata leggermente più fuorviante del previsto.
\clearpage
...
\begin{longtable}{|>{\centering}p{3cm}|>{\centering\arraybackslash}p{3cm}|>{\centering\arraybackslash}p{3.5cm}|>{\centering\arraybackslash}p{3cm}|}
    ...
    \hline
\end{longtable}
\textbf{\glossterm{PB}:} Di 39 file esaminati, 36 sono stati valutati con un classe "A" (con livello di complessità 1-5), mentre 3 con classe "B" (con livello di complessità 6-10). Facendo dunque una media dei valori possibili delle due classi, si è valutato che l'intero progetto sia di classe "A".\\ Ciò vuol dire non solo che la complessità è ben al di sotto del valore accettabile, ma anche del valore ottimale, rendendo il codice di facile comprensione.
...
\begin{longtable}{|>{\centering}p{3cm}|>{\centering\arraybackslash}p{3cm}|>{\centering\arraybackslash}p{3.5cm}|>{\centering\arraybackslash}p{3cm}|}
    ...
    \hline
\end{longtable}
\textbf{\glossterm{PB}:} Esaminando il codice è risultato una percentuale di rating intorno al 93.7\%, con imperfezioni dovute prevalentemente a problemi di cattiva identazione o per stringhe di documentazioni mancanti. Ciò ha impedito di ottenere un risultato dal team considerato sia accettabile che ottimale.
\clearpage
\end{document}
