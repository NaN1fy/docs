...
\begin{longtable}{|>{\centering}p{2cm}|>{\RaggedRight}m{12cm}|>{\centering\arraybackslash}p{2cm}|}
    ...
    TU-01 & Verificare che la classe \textit{SimlatorControllerFactory} crei correttamente un'istanza della classe \textit{SimulatorController} per ogni \textit{StreamWriter}.  & V \\
    \hline

    TU-02 & Verificare che la classe \textit{SimulatorController} funzioni correttamente. Nello specifico, si esaminano i metodi \textit{start\_all} e \textit{stop\_all}.  & V \\
    \hline

    TU-03 & Verificare che la classe \textit{SimulatorThread} adoperi correttamente. & V \\
    \hline

    TU-04 & Verificare che la classe \textit{TemperatureSensor} restituisca la stringa in formato \textit{JSON} attesa. & V \\
    \hline

    TU-05 & Verificare che la classe \textit{HumiditySensor} restituisca la stringa in formato \textit{JSON} attesa. & V \\
    \hline

    TU-06 & Verificare che la classe \textit{ParkingSensor} restituisca la stringa in formato \textit{JSON} attesa. & NI \\
    \hline

    TU-07 & Verificare che le istanze della classe \textit{KafkaProducer} adoperino nelle modalità attese nella produzione di dati, sia nel caso di \textit{success} che nel caso di \textit{failure}. & V \\
    \hline

    TU-08 & Verificare che la funzione \textit{write} della classe \textit{KafkaStreamWriter} funzioni correttamente. & V \\
    \hline

    TU-09 & Verificare che la funzione \textit{write} della classe \textit{StdoutStreamWriter} funzioni correttamente. & V \\
    \hline

    TU-10 & Verificare che la funzione \textit{acked} funzioni correttamente sia nel caso di \textit{success} che nel caso di \textit{failure}. & V \\
    \hline

    TU-11 & Verificare che la classe \textit{Coordinates} funzioni correttamente. & V \\
    \hline

    TU-12 & Verificare che la funzione \textit{jsonfy} funzioni correttamente. & V \\
    \hline
...
\begin{longtable}{|>{\centering}p{2cm}|>{\RaggedRight}m{12cm}|>{\centering\arraybackslash}p{2cm}|}
    ...
    \endlastfoot 
        ...
        TI-01 & Verificare la persistenza dei dati della classe \textit{TemperatureSensor} nel \glossterm{database} \glossterm{ClickHouse}.  & V \\
        \hline
    
        TI-02 & Verificare la persistenza dei dati della classe \textit{HumiditySensor} nel \glossterm{database} \glossterm{ClickHouse}.  & V \\
        \hline

		TI-03 & Verificare la persistenza dei dati della classe \textit{ParkingeSensor} nel \glossterm{database} \glossterm{ClickHouse}. & NI \\
		\hline
