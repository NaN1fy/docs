...
\begin{longtable}{|>{\centering}p{2cm}|>{\RaggedRight}m{12cm}|>{\centering\arraybackslash}p{2cm}|}
    ...
    TU-07 & Verificare che le istanze della classe \verb|KafkaProducer| adoperino nelle modalità attese nella produzione di dati, sia nel caso di success che nel caso di failure. & V \\
    \hline

    TU-08 & Verificare che la funzione \verb|write| della classe \verb|KafkaStreamWriter| funzioni correttamente. & V \\
    \hline

    TU-09 & Verificare che la funzione \verb|write| della classe
    \verb|StdoutStreamWriter| funzioni correttamente. & V \\
    \hline

    TU-10 & Verificare che la funzione \verb|acked| funzioni correttamente sia nel caso di success che nel caso di failure. & V \\
    \hline

    TU-11 & Verificare che la classe \verb|Coordinates| funzioni correttamente. & V \\
    \hline

    TU-12 & Verificare che la funzione \verb|jsonfy| funzioni correttamente. & V \\
    \hline
...
\begin{longtable}{|>{\centering}p{2cm}|>{\RaggedRight}m{12cm}|>{\centering\arraybackslash}p{2cm}|}
    ...
    \endlastfoot 
		...
		TI-03 & Verificare la persistenza dei dati della classe \verb|ParkingeSensor| nel \glossterm{database} \glossterm{ClickHouse}. & NI \\
		\hline
