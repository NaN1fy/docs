% changelog: "0.3.0, 2024-05-14, Davide Donanzan, Stesura Test di Unità e Integrazione"
...
Ogni test è associato ad un codice univoco definito nel seguente formato:
\begin{center}
	\textbf{T[Tipologia]-[Numero]}
\end{center}
Dove \textbf{Tipologia} indica la tipologia del test: 
\begin{itemize}
	\item \textbf{U:} di unità;
...
\begin{table}[h]
    ...
    \begin{tabular}{|p{2cm}|>{\raggedright}M{10cm}|p{2cm}|}
        \hline
        \textbf{Codice Test} & \textbf{Descrizione} & \textbf{Stato Test} \\
        \hline
        
        TU01 & Verificare che la classe \textit{SimlatorControllerFactory} crei correttamente un'istanza della classe \textit{SimulatorController} per ogni \textit{StreamWriter}.  & V \\
        \hline
    
        TU02 & Verificare che la classe \textit{SimulatorController} funzioni correttamente. Nello specifico, si esaminano i metodi \textit{start\_all} e \textit{stop\_all}.  & V \\
        \hline
    
        TU03 & Verificare che la classe \textit{SimulatorThread} adoperi correttamente. & V \\
        \hline
    
        TU04 & Verificare che la classe \textit{TemperatureSensor} restituisca la stringa in formato \textit{JSON} attesa. & V \\
        \hline
    
        TU05 & Verificare che la classe \textit{HumiditySensor} restituisca la stringa in formato \textit{JSON} attesa. & V \\
        \hline

		TU06 & Verificare che la classe \textit{ParkingSensor} restituisca la stringa in formato \textit{JSON} attesa. & NI \\
        \hline		
    
        TU07 & Verificare che le istanze della classe \textit{KafkaProducer} adoperino nelle modalità attese nella produzione di dati, sia nel caso di \textit{success} che nel caso di \textit{failure}. & V \\
        \hline
    
        TU08 & Verificare che la funzione \textit{write} della classe \textit{KafkaStreamWriter} funzioni correttamente. & V \\
        \hline
    
        TU09 & Verificare che la funzione \textit{write} della classe \textit{StdoutStreamWriter} funzioni correttamente. & V \\
        \hline

		TU10 & Verificare che la funzione \textit{acked} funzioni correttamente sia nel caso di \textit{success} che nel caso di \textit{failure}. & V \\
        \hline

		TU11 & Verificare che la classe \textit{Coordinates} funzioni correttamente. & V \\
        \hline

		TU12 & Verificare che la funzione \textit{jsonfy} funzioni correttamente. & V \\
        \hline
    \end{tabular}
...
\begin{table}[h]
    ...
    \begin{tabular}{|p{2cm}|>{\raggedright}M{10cm}|p{2cm}|}
        ...
        TI01 & Verificare la persistenza dei dati della classe \textit{TemperatureSensor} nel \glossterm{database} \glossterm{ClickHouse}.  & V \\
        \hline
    
        TI02 & Verificare la persistenza dei dati della classe \textit{HumiditySensor} nel \glossterm{database} \glossterm{ClickHouse}.  & V \\
        \hline

		TI03 & Verificare la persistenza dei dati della classe \textit{ParkingeSensor} nel \glossterm{database} \glossterm{ClickHouse}. & NI \\
		\hline
    \end{tabular}
