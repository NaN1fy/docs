% changelog: "0.6.0, 2024-05-30, Davide Donanzan, Stesura ulteriori casi d'uso"
...
del progetto SyncCity a seguito di un'attenta analisi del capitolato C6 presentato dall'azienda \glossterm{Proponente} SyncLab e di una successiva discussione con essa attraverso gli incontri svolti.
...
struttura è descritta alla sezione vattelappesca del documento \textit{Norme di Progetto v1.0.0}. %INSERIRE RIMANDO SEZIONE CORRETTA
\subsection{Attori}
...
\begin{itemize}
    ...
    \item \textbf{Postcondizioni:} l'amministratore pubblico visualizza un panello contenente una mappa che mostra la posizione dei sensori;
    \item \textbf{Scenario principale:}
    \begin{enumerate}
      \item L’amministratore pubblico accede alla piattaforma di visualizzazione;
      \item L’amministratore pubblico seleziona la visualizzazione della dashboard relativa ai sensori.
...
\begin{itemize}
    ...
    \item \textbf {Specializzazioni:} [UC-2.1], [UC-2.2], [UC-2.3], [UC-2.4], [UC-2.5], [UC-2.6], [UC-2.7], [UC-2.8], [UC-2.9];  
    \item \textbf{Estensioni:} [UC-5].
\end{itemize}
\begin{figure}[ht!]
    \centering
...
\begin{itemize}
    ...
    \item \textbf{Precondizioni: }l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio ambientale;
    \item \textbf{Postcondizioni:} L’amministratore pubblico visualizza un pannello con un grafico che mostra la media della temperatura, in gradi Celsius (°C), in formato di diagramma di Gauge considerando tutti i sensori attivi nell'intervallo di tempo all'interno della dashbaord.
    \item \textbf{Scenario principale}:
    \begin{enumerate}
    \item L’amministratore pubblico accede alla piattaforma di visualizzazione;
    \item L’amministratore pubblico seleziona la visualizzazione della dashboard relativa al dominio
...
\end{itemize}
\subsubsection*{UC-2.5: Visualizzazione pannello con grafico a barre per valore medio, moda, valore minimo e valore massimo di temperatura}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-2.5: Visualizzazione pannello bar chart per valore medio, moda, valore minimo e valore massimo di temperatura}
\begin{itemize}
    \item \textbf{Attore principale:} amministratore pubblico;
    \item \textbf{Precondizioni: }l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio ambientale;
    \item \textbf{Postcondizioni:} L’amministratore pubblico visualizza un pannello con un grafico che mostra il valore medio, minimo, massimo e la moda di temperatura, in gradi Celsius (°C), in formato di diagramma a barre considerando per ciascun sensore attivo nell'intervallo di tempo all'interno della dashbaord.
    \item \textbf{Scenario principale}:
    \begin{enumerate}
    \item L’amministratore pubblico accede alla piattaforma di visualizzazione;
    \item L’amministratore pubblico seleziona la visualizzazione della dashboard relativa al dominio
        ambientale; 
    \item L’amministratore pubblico seleziona la visualizzazione del pannello per il valore medio, moda, valore minimo e valore massimo di temperatura.
    \end{enumerate}
...
\end{itemize}
\subsubsection*{UC-2.6: Visualizzazione pannello con grafico a barre per temperatura percepita media, minima e massima}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-2.6: Visualizzazione pannello con grafico a barre per temperatura percepita media, minima e massima}
\begin{itemize}
    \item \textbf{Attore principale:} amministratore pubblico;
    \item \textbf{Precondizioni: }l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio ambientale;
    \item \textbf{Postcondizioni:} L’amministratore pubblico visualizza un pannello con un grafico che mostra la media, minima e massima temperatura percepita, in gradi Celsius (°C), in formato di diagramma a barre per ciascun sensore attivo nell'intervallo di tempo all'interno della dashbaord.
    \item \textbf{Scenario principale}:
    \begin{enumerate}
    \item L’amministratore pubblico accede alla piattaforma di visualizzazione;
    \item L’amministratore pubblico seleziona la visualizzazione della dashboard relativa al dominio
        ambientale; 
    \item L’amministratore pubblico seleziona la visualizzazione del pannello per temperatura percepita media, minima e massima.
        media.
    \end{enumerate}
\item \textbf{User Story associata:} in qualità di amministratore pubblico, desidero accedere al pannello per monitorare la temperatura percepita media, minima e massima in ogni zona d'interesse della città.
\end{itemize}
\subsubsection*{UC-2.7: Visualizzazione pannello time series per precipitazioni}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-2.7: Visualizzazione pannello time series per precipitazioni}
\begin{itemize}
    \item \textbf{Attore principale:} amministratore pubblico;
    \item \textbf{Precondizioni: }l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio ambientale;
...
\end{itemize}
\subsubsection*{UC-2.8: Visualizzazione pannello time series per inquinamento dell'aria}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-2.8: Visualizzazione pannello time series per inquinamento dell'aria}
\begin{itemize}
    \item \textbf{Attore principale:} amministratore pubblico;
    \item \textbf{Precondizioni: }l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio ambientale;
    \item \textbf{Postcondizioni:} L’amministratore pubblico visualizza un pannello con un grafico
        che mostra la quantità di polveri sottili presenti nell'aria, espressa in microgrammi per metro cubo ($\mu g / m^3$), in formato time series.
    \item \textbf{Scenario principale}:
    \begin{enumerate}
    \item L’amministratore pubblico accede alla piattaforma di visualizzazione;
    \item L’amministratore pubblico seleziona la visualizzazione della dashboard relativa al dominio
...
\end{itemize}
\subsubsection*{UC-2.9: Visualizzazione pannello time series per livello di acqua}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-2.9: Visualizzazione pannello time series per livello di acqua}
\begin{itemize}
    \item \textbf{Attore principale:} amministratore pubblico;
    \item \textbf{Precondizioni: }l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio ambientale;
...
\begin{figure}[ht!]
    \centering
        \begin{tikzpicture}
            \umlactor[x=0, y=-3, inner xsep=3em, inner ysep=3em]{Amministratore pubblico}
            \begin{umlsystem}[x=9, y=0]{UC-2 Visualizzazione dashboard ambientale}
                ...
                \umlusecase[x=0, y=-10.5, width=100, name=UC-temp-stat]{UC-2.5 \\ Visualizzazione pannello bar chart per valore medio, minimo, massimo e moda della temperatura}
            \end{umlsystem}
...
\begin{figure}[ht!]
    \centering
        \begin{tikzpicture}
            \umlactor[x=0, y=-3, inner xsep=3em, inner ysep=3em]{Amministratore pubblico}
            \begin{umlsystem}[x=9, y=0]{UC-2 Visualizzazione dashboard ambientale}
                \umlusecase[x=0, y=0, width=100, name=UC-heat-stat]{UC-2.6 \\ Visualizzazione pannello bar chart per media, minima e massima temperatura percepita}

                \umlusecase[x=0, y=-2.5, width=100, name=UC-rain]{UC-2.7 \\ Visualizzazione pannello time series per precipitazioni}

                \umlusecase[x=0, y=-5, width=100, name=UC-pollut]{UC-2.8 \\ Visualizzazione pannello time series per inquinamento dell'aria}

                \umlusecase[x=0, y=-7.5, width=100, name=UC-water]{UC-2.9 \\ Visualizzazione pannello time series per livello di acqua}
            \end{umlsystem}
...
\end{figure}
\subsubsection*{UC-3.1: Visualizzazione pannello tabellare per disponibilità parcheggi}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-3.1: Visualizzazione pannello
tabellare per disponibilità parcheggi}
\begin{itemize}
    \item \textbf{Attore principale:} amministratore pubblico;
    \item \textbf{Precondizioni:} l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio urbanistico.
    \item \textbf{Postcondizioni:} l'amministratore pubblico visualizza un pannello che mostra lo
        stato di occupazione dei parcheggi, in formato tabellare;
    \item \textbf{Scenario principale:} 
    \begin{enumerate}
    \item L’amministratore pubblico accede alla piattaforma di visualizzazione;
    \item L’amministratore pubblico seleziona la visualizzazione della dashboard relativa al dominio
        urbanistico;
    \item L’amministratore pubblico seleziona la visualizzazione del pannello tabellare per lo stato di occupazione dei parcheggi.
    \end{enumerate}
...
\end{itemize}
\subsubsection*{UC-3.2: Visualizzazione pannello in formato di registro per notifiche pagamento parcheggi}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-3.2: Visualizzazione pannello in formato di registro per notifiche pagamento parcheggi}
\begin{itemize}
    \item \textbf{Attore principale:} amministratore pubblico;
    \item \textbf{Precondizioni:} l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio urbanistico.
...
\end{itemize}
\subsubsection*{UC-3.3: Visualizzazione pannello tabellare per guasti elettrici}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-3.3: Visualizzazione pannello
tabellare per guasti elettrici}
\begin{itemize}
    \item \textbf{Attore principale:} amministratore pubblico;
    \item \textbf{Precondizioni:} l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio urbanistico.
    \item \textbf{Postcondizioni:} l'amministratore pubblico visualizza un pannello che mostra i
        guasti sulla rete elettrica in formato tabellare;
    \item \textbf{Scenario principale:} 
    \begin{enumerate}
    \item L’amministratore pubblico accede alla piattaforma di visualizzazione;
    \item L’amministratore pubblico seleziona la visualizzazione della dashboard relativa al dominio urbanistico;
    \item L’amministratore pubblico seleziona la visualizzazione del pannello tabellare per i guasti.
    \end{enumerate}
...
\end{itemize}
\subsubsection*{UC-3.4: Visualizzazione pannello time series per riempimento isole ecologiche}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-3.4: Visualizzazione pannello time series per stato riempimento isole ecologiche}
\begin{itemize}
    \item \textbf{Attore principale:} amministratore pubblico;
    \item \textbf{Precondizioni:} l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio urbanistico.
...
\end{itemize}
\subsubsection*{UC-3.5: Visualizzazione pannello tabellare per disponibilità colonnine di ricarica}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-3.5: Visualizzazione pannello
...
\begin{itemize}
    ...
    \item \textbf{Precondizioni:} l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio urbanistico.
    \item \textbf{Postcondizioni:} l'amministratore pubblico visualizza un pannello che mostra lo
        stato di occupazione delle colonnine di ricarica, in formato tabellare;
    \item \textbf{Scenario principale:} 
    \begin{enumerate}
    \item L’amministratore pubblico accede alla piattaforma di visualizzazione;
    \item L’amministratore pubblico seleziona la visualizzazione della dashboard relativa al dominio urbanistico;
    \item L’amministratore pubblico seleziona la visualizzazione del pannello mappa interattiva per la disponibilità delle colonnine di ricarica.
    \end{enumerate}
...
\end{itemize}
\subsubsection*{UC-3.6: Visualizzazione pannello time series per consumi colonnine di ricarica}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-3.6: Visualizzazione pannello time series per consumi colonnine di ricarica}
\begin{itemize}
    \item \textbf{Attore principale:} amministratore pubblico;
    \item \textbf{Precondizioni:} l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio urbanistico.
...
\end{itemize}
\subsubsection*{UC-3.7: Visualizzazione pannello in formato di registro per notifiche pagamento colonnine di ricarica}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-3.2: Visualizzazione pannello in formato di registro per notifiche pagamento colonnine di ricarica}
\begin{itemize}
    \item \textbf{Attore principale:} amministratore pubblico;
    \item \textbf{Precondizioni:} l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio urbanistico.
...
\end{itemize}
\subsubsection*{UC-3.8: Visualizzazione pannello bar gauge per dati pagamento parcheggio}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-3.8: Visualizzazione pannello bar gauge per dati pagamento parcheggio}
\begin{itemize}
    \item \textbf{Attore principale:} amministratore pubblico;
    \item \textbf{Precondizioni:} l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio urbanistico.
    \item \textbf{Postcondizioni:} l'amministratore pubblico visualizza un pannello che mostra il
        costo medio, massimo e minimo dei parcheggi;
    \item \textbf{Scenario principale:} 
    \begin{enumerate}
    \item L’amministratore pubblico accede alla piattaforma di visualizzazione;
    \item L’amministratore pubblico seleziona la visualizzazione della dashboard relativa al dominio urbanistico;
...
\end{itemize}
\subsubsection*{UC-3.9: Visualizzazione pannello bar gauge per dati pagamento di colonnine di ricarica}
\addcontentsline{toc}{subsubsection}{\protect\numberline{}UC-3.8: Visualizzazione pannello bar gauge per dati pagamento di colonnine di ricarica}
\begin{itemize}
    \item \textbf{Attore principale:} amministratore pubblico;
    \item \textbf{Precondizioni:} l’amministratore pubblico ha selezionato la visualizzazione
        relativa al dominio urbanistico.
    \item \textbf{Postcondizioni:} l'amministratore pubblico visualizza un pannello che mostra il
        costo medio, massimo e minimo dei pagamenti per le colonnine di ricarica;
    \item \textbf{Scenario principale:} 
    \begin{enumerate}
    \item L’amministratore pubblico accede alla piattaforma di visualizzazione;
    \item L’amministratore pubblico seleziona la visualizzazione della dashboard relativa al dominio urbanistico;
    ...
    \centering
        \begin{tikzpicture}
            ...
            \begin{umlsystem}[x=9, y=0]{UC-3 Visualizzazione dashboard urbanistica}
                \umlusecase[x=0, y=0, width=100, name=UC-park]{UC-3.1 \\ Visualizzazione pannello
                tabellare per disponibilità parcheggi}

                \umlusecase[x=0, y=-2.5, width=100, name=UC-paypark]{UC-3.2 \\ Visualizzazione
                pannello in formato di registro per notifiche pagamento parcheggi}

                \umlusecase[x=0, y=-5, width=100, name=UC-elec]{UC-3.3 \\ Visualizzazione pannello
                tabellare per guasti elettrici}

                \umlusecase[x=0, y=-7.5, width=100, name=UC-eco]{UC-3.4 \\ Visualizzazione pannello time series per riempimento isole ecologiche}
            \end{umlsystem}
                \umlassoc{Amministratore pubblico}{UC-park}
...
\begin{figure}[ht!]
    \centering
        \begin{tikzpicture}
            \umlactor[x=0, y=-3, inner xsep=3em, inner ysep=3em]{Amministratore pubblico}
            \begin{umlsystem}[x=9, y=0]{UC-3 Visualizzazione dashboard urbanistica}
                \umlusecase[x=0, y=0, width=100, name=UC-charge]{UC-3.5 \\ Visualizzazione pannello tabellare per disponibilità colonnine di ricarica}

                \umlusecase[x=0, y=-2.5, width=100, name=UC-cons]{UC-3.6 \\ Visualizzazione pannello time series per consumi colonnine di ricarica}

                \umlusecase[x=0, y=-5, width=100, name=UC-paycharge]{UC-3.7 \\ Visualizzazione pannello in formato di registro per notifiche pagamento colonnine di ricarica}
                
                \umlusecase[x=0, y=-7.5, width=100, name=UC-paypark2]{UC-3.8 \\ Visualizzazione pannello bar gauge per dati pagamento parcheggio }

                \umlusecase[x=0, y=-10, width=100, name=UC-paycharge2]{UC-3.9 \\ Visualizzazione pannello bar gauge per dati pagamento colonnine di ricarica }
            \end{umlsystem}
                \umlassoc{Amministratore pubblico}{UC-charge}
                ...
                \umlassoc{Amministratore pubblici}{UC-paycharge2}
        \end{tikzpicture}
...
\begin{longtable}{|>{\centering\arraybackslash}p{1.2cm}|>{\centering\arraybackslash}p{2cm}|>{\centering\arraybackslash}p{8.5cm}|>{\centering\arraybackslash}p{3cm}|}
 ...
 \endlastfoot
            ...
            RF-10 & Obbligatorio & Il sistema deve integrare almeno un sensore che invii dati di pagamento. & \glossterm{Proponente} \\ \hline
            ...
            RF-14 & Obbligatorio & L'utente deve poter visualizzare un menu attraverso il quale selezionare la \glossterm{dashboard} desiderata tra Sensori, Ambientale ed Urbanistica. & UC-0 \\ \hline
            RF-15 & Obbligatorio & L'utente deve poter visualizzare la \glossterm{dashboard} relativa allo stato dei sensori. & UC-1 \\ \hline
            RF-16 & Obbligatorio & L'utente deve poter visualizzare un \glossterm{widget} contente una mappa su cui è indicata la posizione geografica dei sensori. & \glossterm{Capitolato}, UC-1.1 \\ \hline
            RF-17 & Obbligatorio & L'utente deve poter visualizzare la \glossterm{dashboard} relativa al dominio ambientale. & UC-2 \\ \hline
            RF-18 & Obbligatorio & L'utente deve poter visualizzare un \glossterm{widget} che mostra le rilevazioni della temperatura in formato time series. & UC-2.1 \\ \hline
            RF-19 & Obbligatorio & L'utente deve poter visualizzare un \glossterm{widget} che mostra le rilevazioni dell'umidità in formato time series. & UC-2.2 \\ \hline
            RF-20 & Desiderabile & L'utente deve poter visualizzare un \glossterm{widget} che mostra le rilevazioni della temperatura percepita in formato time series. & UC-2.3 \\ \hline
            RF-21 & Desiderabile & L'utente deve poter visualizzare un \glossterm{widget} che mostra la temperatura media in formato diagramma di Gauge considerando tutti i sensori attivi nell'intervallo di tempo. & UC-2.4 \\ \hline
            RF-22 & Obbligatorio & L'utente deve poter visualizzare un \glossterm{widget} che mostra l'intensità delle precipitazioni in formato time series. & UC-2.5 \\ \hline
            RF-23 & Obbligatorio & L'utente deve poter visualizzare un \glossterm{widget} che mostra l'inquinamento dell'aria in formato time series. & UC-2.6 \\ \hline
            RF-24 & Obbligatorio & L'utente deve poter visualizzare un \glossterm{widget} che mostra il livello dell'acqua in formato time series. & UC-2.7 \\ \hline
            RF-25 & Obbligatorio & L'utente deve poter visualizzare la \glossterm{dashboard} relativa al dominio urbanistico. & UC-3 \\ \hline
            RF-26 & Obbligatorio & L'utente deve poter visualizzare un \glossterm{widget} che mostra una tabella che descrive lo stato di occupazione dei parcheggi. & UC-3.1 \\ \hline
            RF-27 & Obbligatorio & L'utente deve poter visualizzare un \glossterm{widget} che mostra una lista di notifiche dei pagamenti dei parcheggi. & UC-3.2 \\ \hline
            RF-28 & Obbligatorio & L'utente deve poter visualizzare un \glossterm{widget} che mostra una tabella che descrive i guasti alla fornitura elettrica. & UC-3.3 \\ \hline
            RF-29 & Obbligatorio & L'utente deve poter visualizzare un \glossterm{widget} che mostra lo stato di riempimento delle isole ecologiche in formato time series. & UC-3.4 \\ \hline
            RF-30 & Obbligatorio & L'utente deve poter visualizzare un \glossterm{widget} che mostra una tabella che descrive la disponibilità colonnine di ricarica. & UC-3.5 \\ \hline
            RF-31 & Desiderabile & L'utente deve poter visualizzare un \glossterm{widget} che mostra i consumi di elettricità delle colonnine di ricarica in formato time series. & UC-3.6 \\ \hline
            RF-32 & Obbligatorio & L'utente deve poter visualizzare un \glossterm{widget} che mostra le notifiche di pagamento relative all'utilizzo delle colonnine di ricarica. & UC-3.7 \\ \hline
            RF-33 & Desiderabile & L'utente deve poter visualizzare la \glossterm{dashboard} relativa al superamento delle soglie. & UC-4 \\ \hline
            RF-34 & Desiderabile & L'utente deve poter visualizzare un \glossterm{widget} che mostra che mostra una lista di notifiche riguardanti il superamento delle soglie di temperatura. & UC-4.1 \\ \hline
            RF-35 & Desiderabile & L'utente deve poter visualizzare un \glossterm{widget} che mostra che mostra una lista di notifiche riguardanti il superamento della soglia di precipitazioni. & UC-4.2 \\ \hline
            RF-36 & Desiderabile & L'utente deve poter visualizzare un \glossterm{widget} che mostra che mostra una lista di notifiche riguardanti il superamento della soglia di inquinamento dell'aria. & UC-4.3 \\ \hline
            RF-37 & Desiderabile & L'utente deve poter visualizzare un \glossterm{widget} che mostra che mostra una lista di notifiche riguardanti il superamento della soglia di livello dell'acqua. & UC-4.4 \\ \hline
            RF-38 & Desiderabile & L'utente deve poter visualizzare un \glossterm{widget} che mostra che mostra una lista di notifiche riguardanti il superamento della soglia di riempimento dei conferitori nelle isole ecologiche. & UC-4.5 \\ \hline
            RF-39 & Obbligatorio & L'utente deve poter visualizzare un messaggio di errore qualora non sia possibile reperire i dati o vi sia un malfunzionamento nel sistema. & UC-5 \\ \hline
            RF-40 & Obbligatorio & L'utente deve poter applicare dei filtri per escludere alcuni dati dalla visualizzazione, secondo determinati criteri. & UC-6 \\ \hline
            RF-41 & Obbligatorio & L'utente deve poter applicare dei filtri per visualizzare solo i dati provenienti da alcune specifiche tipologie di sensori. & UC-6.1 \\ \hline
            RF-42 & Obbligatorio & L'utente deve poter applicare dei filtri per visualizzare solo i dati misurati in un determinato intervallo temporale. & UC-6.2 \\ \hline
            RF-43 & Obbligatorio & L'utente deve poter rimuovere i filtri applicati in precedenza in modo da non escludere alcun dato dalla visualizzazione. & UC-7 \\ \hline
            RF-44 & Desiderabile & L'utente deve poter modificare il layout dei \glossterm{widget} a proprio piacimento. & UC-8 \\ \hline
            RF-45 & Desiderabile & L'utente deve poter ridimensionare i \glossterm{widget} a proprio piacimento. & UC-8.1 \\ \hline
            RF-46 & Desiderabile & L'utente deve poter spostare i \glossterm{widget} a proprio piacimento. & UC-8.2 \\ \hline
            RF-47 & Obbligatorio & Il sensore deve poter inviare i dati misurati, assieme al timestamp di rilevazione e alle proprie coordinate geografiche. & UC-9 \\ \hline
            RF-48 & Obbligatorio & Il sensore deve poter inviare i dati relativi alla temperatura, assieme al timestamp di rilevazione e alle proprie coordinate geografiche. & UC-9.1 \\ \hline
            RF-49 & Obbligatorio & Il sensore deve poter inviare i dati relativi all'umidità, assieme al timestamp di rilevazione e alle proprie coordinate geografiche. & UC-9.2 \\ \hline
            RF-50 & Obbligatorio & Il sensore deve poter inviare i dati relativi alle precipitazioni, assieme al timestamp di rilevazione e alle proprie coordinate geografiche. & UC-9.3 \\ \hline
            RF-51 & Obbligatorio & Il sensore deve poter inviare i dati relativi all'inquinamento atmosferico, assieme al timestamp di rilevazione e alle proprie coordinate geografiche. & UC-9.4 \\ \hline
            RF-52 & Obbligatorio & Il sensore deve poter inviare i dati relativi al livello dell'acqua, assieme al timestamp di rilevazione e alle proprie coordinate geografiche. & UC-9.5 \\ \hline
            RF-53 & Obbligatorio & Il sensore deve poter inviare i dati relativi allo stato di occupazione dei parcheggi, assieme al timestamp di rilevazione e alle proprie coordinate geografiche. & UC-9.6 \\ \hline
            RF-54 & Obbligatorio & Il sensore deve poter inviare i dati relativi ai pagamenti dei parcheggi, assieme al timestamp di rilevazione e alle proprie coordinate geografiche. & UC-9.7 \\ \hline
            RF-55 & Obbligatorio & Il sensore deve poter inviare i dati relativi ai guasti elettrici, assieme al timestamp di rilevazione e alle proprie coordinate geografiche. & UC-9.8 \\ \hline
            RF-56 & Obbligatorio & Il sensore deve poter inviare i dati relativi al livello di riempimento dei conferitori nelle isole ecologiche, assieme al timestamp di rilevazione e alle proprie coordinate geografiche. & UC-9.9 \\ \hline
            RF-57 & Obbligatorio & Il sensore deve poter inviare i dati relativi allo stato di occupazione delle colonnine di ricarica, assieme al timestamp di rilevazione e alle proprie coordinate geografiche. & UC-9.10 \\ \hline
            RF-58 & Obbligatorio & Il sensore deve poter inviare i dati relativi ai consumi di elettricità delle colonnine di ricarica, assieme al timestamp di rilevazione e alle proprie coordinate geografiche. & UC-9.11 \\ \hline
            RF-59 & Obbligatorio & Il sensore deve poter inviare i dati relativi ai pagamenti delle ricariche effettuate mediante colonnine elettriche, assieme al timestamp di rilevazione e alle proprie coordinate geografiche. & UC-9.12 \\ \hline
            RF-60 & Obbligatorio & Il sistema consentire la correlazione di dati provenienti da sensori diversi. & \glossterm{Proponente} \\
		\hline
\end{longtable}
...
\begin{longtable}{|>{\centering\arraybackslash}p{1.2cm}|>{\centering\arraybackslash}p{2cm}|>{\centering\arraybackslash}p{8.5cm}|>{\centering\arraybackslash}p{3cm}|}
    ...
    \endlastfoot
            RQ-1 & Obbligatorio & Devono essere rispettati i vincoli e le metriche descritti nel documento \textit{Piano di Qualifica v1.0.0}. & \textit{Norme di Progetto} \\ \hline
            RQ-2 & Obbligatorio & Devono essere rispettate le norme specificate nel documento \textit{Norme di Progetto v1.0.0}. & Norme di Progetto \\ \hline
            RQ-3 & Obbligatorio & Il prodotto deve superare i test che provano il corretto funzionamento dei servizi impiegati e delle funzionalità implementate. La copertura di test deve essere almeno dell'80\% e deve essere adeguatamente documentata. & \glossterm{Capitolato} \\ \hline
            RQ-4 & Obbligatorio & La \glossterm{Proponente} deve essere in grado di accedere alla \glossterm{repository} GitHub del prodotto ``SyncCity". & \glossterm{Proponente} \\ \hline
...
\begin{longtable}{|>{\centering\arraybackslash}p{1.2cm}|>{\centering\arraybackslash}p{2cm}|>{\centering\arraybackslash}p{8.5cm}|>{\centering\arraybackslash}p{3cm}|}
    ...
    \endlastfoot
            RV-1 & Obbligatorio & La persistenza dei dati deve essere realizzata grazie ad un database OLAP colonnare come \glossterm{ClickHouse}. & \glossterm{Capitolato} \\ \hline
            ...
            RV-3 & Obbligatorio & La visualizzazione dei dati deve essere consentita utilizzando una piattaforma di data-visualization come Grafana. & \glossterm{Capitolato} \\ \hline
            ...
            RV-5 & Obbligatorio & I dati generati dal simulatore devono essere in formato JSON. & \textit{Norme di Progetto} \\ \hline
\end{longtable}
...
\begin{longtable}{|>{\centering\arraybackslash}p{4cm}|>{\centering\arraybackslash}p{4cm}|}
 ...
 \endlastfoot
            ...
            RF-14 & UC-0 \\ \hline
            RF-15 & UC-1 \\ \hline
            RF-16 & \glossterm{Capitolato}, UC-1.1 \\ \hline
            RF-17 & UC-2 \\ \hline
            RF-18 & UC-2.1 \\ \hline
            RF-19 & UC-2.2 \\ \hline
            RF-20 & UC-2.3 \\ \hline
            ...
            RF-58 & UC-9.11 \\ \hline
            RF-59 & UC-9.12 \\ \hline
            RF-60 & \glossterm{Proponente} \\ \hline
            RQ-1 & \textit{Norme di Progetto} \\ \hline
            RQ-2 & \textit{Norme di Progetto} \\ \hline
            ...
            RV-5 & \textit{Norme di Progetto} \\ \hline
...
\rowcolor{white}
  \caption{Tracciamento fonte - requisito.}
	\label{table:Tracciamento requisito - fonte}
 \endlastfoot
            Capitolato & RF-1, RF-2, RF-3, RF-4, RF-5, RF-6, RF-7, RF-8, RF-9, RF-11, RF-12, RF-13, RF-16, RQ-3, RQ-5, RQ-6, RQ-7, RV-1, RV-2, RV-3 \\ \hline
        Norme di Progetto & RQ-1, RQ-2, RV-5 \\ \hline
            \glossterm{Proponente} & RF-10, RF-59, RQ-4, RV-4, RP-1, RP-2, RP-3 \\ \hline
            UC-0 & RF-14 \\ \hline
            UC-1 & RF-15 \\ \hline
            UC-1.1 & RF-16 \\ \hline
            UC-2 & RF-17\\ \hline
